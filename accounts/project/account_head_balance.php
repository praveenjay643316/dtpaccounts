...
class Account_head_balance extends ConfigClass
{
    public function voucherFieldNames($voucher_type)
    {
        switch ($voucher_type) {
            ...
            case 4:
                ...
                $voucher_breakup_table = "accounts_master.t_pt_voucher_breakup";
    ...
    public function getVoucherBreakUpIdsDetails($voucher_type, $chalan_no)
    {

        $lbcode = $this->getCurrentLocalBodyCode();
        $dcode = $this->getCurrentDistrictCode();
        $voucher_fields = $this->voucherFieldNames(voucher_type: $voucher_type);
        $query = "select ".$voucher_fields['credit_account_id_field'].",".$voucher_fields['debit_account_id_field']." from " . $voucher_fields['voucher_table'] . " where " . $voucher_fields["chalan_no_field"] . "=:chalan_no and del_flag is null and lbcode=:lbcode and dcode=:dcode";
        $res = $this->prepare($query, [":chalan_no" => $chalan_no, ":lbcode" => $lbcode, ":dcode" => $dcode], 2);
        $breakup_ids = [];
        foreach ($res as $row) {
            //return int $amount,int $account_head_id,int $type,bool

            $curr_credit_breakup_id = json_decode($row[$voucher_fields['credit_account_id_field']]);
            $curr_debit_breakup_id = json_decode($row[$voucher_fields['debit_account_id_field']]);
            //print_r(['credit'=>$curr_credit_breakup_id,"debit"=>$curr_debit_breakup_id]);die;
            ...
            . $voucher_fields['breakup_id_field'] . " in " . "(" . implode(',', $breakup_ids) . ") and jv.del_flag is null and jv.lbcode=:lbcode and jv.dcode=:dcode";

            $res2 = $this->prepare($query, [":lbcode" => $lbcode, ":dcode" => $dcode], 2);
            $return_arr = [];
    ...
    public function getBRVBreakUpIdsDetails($voucher_no)
    {
        $lbcode = $this->getCurrentLocalBodyCode();
        $dcode = $this->getCurrentDistrictCode();
        $fin_year=$this->getFinYear();
        //$voucher_fields = $this->voucherFieldNames(voucher_type: $voucher_type);
        $query = "select credit_breakup_id,debit_breakup_id from account_masters.t_bank_receipt_voucher where brv_chalan_no=:voucher_no and del_flag is null and lbcode=:lbcode and dcode=:dcode and fin_year=:fin_year";
        $res = $this->prepare($query, [":voucher_no" => $voucher_no, ":lbcode" => $lbcode, ":dcode" => $dcode,":fin_year"=>$fin_year], 2);
        $return_arr = [];
        foreach ($res as $row) {
            //return int $amount,int $account_head_id,int $type,bool
            ...
            $query = "select acc_head.account_head_id,jv.credit_amount,jv.debit_amount from accounts_master.t_bank_receipt_voucher_breakup as jv left join accounts_master.m_account_head as acc_head on (acc_head.account_head_id=jv.acc_code_id) where  brv_breakup_id  in ( ". implode(',', $breakup_ids) . ") and jv.del_flag is null and jv.lbcode=:lbcode and jv.dcode=:dcode";
    ...
    public function getBPVBreakUpIdsDetails($voucher_no)
    {
        ...
        $fin_year=$this->getFinYear();
        //$voucher_fields = $this->voucherFieldNames(voucher_type: $voucher_type);
        $query = "select acc_code_ids from accounts_master.t_bank_payment_voucher where bpv_chalan_no=:voucher_no and del_flag is null and lbcode=:lbcode and dcode=:dcode and fin_year=:fin_year";
        $res = $this->prepare($query, [":voucher_no" => $voucher_no, ":lbcode" => $lbcode, ":dcode" => $dcode,":fin_year"=>$fin_year], 2);
        $return_arr = [];
        foreach ($res as $row) {
            //return int $amount,int $account_head_id,int $type,bool
            ...
            //$curr_credit_breakup_id = json_decode($row['credit_account_id']);
            //$curr_debit_breakup_id = json_decode($row['debit_account_id']);
            $breakup_ids = json_decode($row['acc_code_ids']);
    ...
    public function getTriplicateBreakUpIdsDetails($chalan_no)
    {
        ...
        {   
           ...
           $breakup_ids = array_merge($curr_credit_breakup_id, $curr_debit_breakup_id);
            ...
            $query = "select acc_head.account_head_id,jv.credit_account_id,jv.credit_amount,jv.debit_account_id,jv.debit_amount from accounts_master.t_triplicate_accounthead_breakup as jv left join accounts_master.m_account_head as acc_head on (acc_head.account_head_id=jv.debit_account_id 
            or acc_head.account_head_id=jv.credit_account_id) where accounthead_breakup_id in " . "(" . implode(',', $breakup_ids) . ") and jv.del_flag is null and jv.lbcode=:lbcode and jv.dcode=:dcode and triplicate_chalan_no=:triplicate_chalan_no" ;

            $res2 = $this->prepare($query, [":lbcode" => $lbcode, ":dcode" => $dcode, ":triplicate_chalan_no"=>$chalan_no], 2);
            $return_arr = [];
            foreach ($res2 as $row2) {
                if ($row2['credit_account_id'] != NULL) {
                    $return_arr[] = ["account_head_id" => $row2['account_head_id'], "amount" => $row2['credit_amount'], "type" => 2];
