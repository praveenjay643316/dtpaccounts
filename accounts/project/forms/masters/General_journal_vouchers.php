...
class AdjustBankReceiptVoucher  extends ConfigClass
{
        ...
        <script type="text/javascript">
            $(document).ready(function() {	
				...
				$(document).on('click', '#btn_debit_edit', function() {
                    ...
                    $.ajax({
						url: "General_journal_vouchers.php",
						type: "post",
						data: {
                            "account_type":btoa(2),
                            "id":btoa(id),
    ...
    document.getElementById('credit_bank_head').value = accountName;
                ...
                <?php if (!isset($post_data_array['del_id'])) { ?>
                   $(document).on('click', "#btn_save", function () {
                    try {
                        ...
                        if ($("#gjv_serial_no").val().length === 0) {
                            throw { msg: "Enter Serial Number", foc: "#gjv_serial_no" };
                        }

                        if ($("#date").val().length === 0) {
                            throw { msg: "Select Date", foc: "#date" };
                        }

                        if ($("#credit_total_amount").val().length === 0) {
                            throw { msg: "Enter Credit Amount", foc: "#credit_amount" };
                        }

                        if ($("#debit_total_amount").val().length === 0) {
                            throw { msg: "Enter Debit Amount", foc: "#debit_amount" };
                        }

                        var debit_amount  = parseFloat($("#debit_total_amount").val());
                        var credit_amount = parseFloat($("#credit_total_amount").val());

                        if (debit_amount !== credit_amount) {
                            throw { msg: "Debit Amount and Credit Amount must be same" };
                        }

                        if ($("#narration").val().length === 0) {
        ...
        <div class="container mt-3">
            ...
            <form action="General_journal_vouchers.php" method="post" enctype="multipart/form-data" autocomplete="off">
                    ...
                    <div class="card-body pl-5 pr-5">
                        ...
                        <table class="table table-bordered m-0 p-0 table-striped tndtp_form_table">
                            <thead class="bg-th-form-dsg">
                                <tr>
                                    <th align="center" scope="col" colspan="12">General Journal Voucher</th>
                                </tr>
                            </thead>
                            <tbody>
                                <tr>
                                    ...
                                    <?php 
                                        ...
                                        ?>
                                         <input type="hidden" id="gjv_chalan_no" name="gjv_chalan_no" class="form-control w-50 form-control-sm" value="<?php echo $chalan_no; ?>"/>
                                        <input type="hidden" id="gjv_serial_no" name="gjv_serial_no" class="form-control w-50 form-control-sm" value="<?php echo $sel_qry_res['id']+1; ?>"/>
                                    </td>
                                </tr>
                                <tr>
...
<select id="debit_bank_code" name="debit_bank_code" class="form-control form-control-sm mb-2">
    ...
    <?php $res = $this->Select_Account_Head_Code(1,3);
//     $query = "
//     SELECT 
...
//         ":isactive"=>1,":voucher_id"=>3,":account_type_id"=>2
//     ],2);
    foreach ($res as $row): ?>
...
<select id="credit_bank_code" name="credit_bank_code" class="form-control form-control-sm mb-2">
    <option value="">Choose</option>
    <?php $res = $this->Select_Account_Head_Code(2,3);
//                 $query = "
//                     SELECT 
//                         li.account_headid as account_head_id,
//                         acc_head.account_code,
...
//                 ";
// $res = $this->prepare($query, [
//         ":isactive"=>1,":voucher_id"=>3,":account_type_id"=>1
//     ], 2);
    foreach ($res as $row): ?>
        <option value="<?= $row['account_head_id']; ?>"
                data-code="<?= htmlentities($row['old_code']); ?>"
...
<input type="hidden" id="credit_bank_head" name="credit_bank_head" class="form-control form-control-sm number_field" />
    ...
    public function data_save($save_data)
    {
        ...
        if (isset($save_data['gjv_chalan_no']) && $save_data['gjv_chalan_no']!='') {
            $gjv_chalan_no = $save_data['gjv_chalan_no'];
            $gjv_chalan_no_Validation = $this->Field_Validation(
                array(
                    'Field_Type' => 'number_slash_hyphen',
                    'Field_Value' => $save_data['gjv_chalan_no'],
                    'Field_Name' => 'BRV Chalan Number',
                    'Field_Label_Name' => 'BRV Chalan Number',
                )
            );

            if ($gjv_chalan_no_Validation['Status'] == "Error") {
                $this->main_content(array_merge(array(
                    "STATUS" => "ERROR",
                    "STATUS_TYPE" => "FIELD",
                    "FIELD_NAME" => "gjv_chalan_no",
                    "MESSAGE" => $gjv_chalan_no_Validation['Message']
                ), $save_data));
                exit;
            }
        }else{
        ...
        $res1 = $this->prepare($save_query, array(":dcode" => $dcode, ":lbcode" => $lbcode, ":gjv_no" => $gjv_serial_no, ":gjv_date" => $date, ":fin_year" => $fin_year, ":debit_amount" => $debit_amount, ":credit_amount" => $credit_amount, ":total_amount" => $credit_amount, ":narration" => $narration, ":user_name" => $user_name, ":ip_address" => $ip_address, ":edit_id" => 0, ":del_id" => 0), 4);
        if (!isset($res1->errorInfo)) {
            ...
            try{
                $account_head_balance_obj->update_voucher_head_amount(3,$gjv_serial_no,False);
                $this->commit();
            }
            catch(Exception $e)
            {
            ...
            exit;
             ...
             $site_data = $this->siteData();
             $url = $site_data->website_url . '/project/forms/masters/General_Journal_Voucher_Reciept.php?gjvno=' . base64_encode($inserted_id);
             header("Location: $url");
             exit; 
...
if (!isset($_POST['cmd'])) {
    ...
    if ($cmd == 3) {
        ...
        $res1 = $AdjustBankReceiptVoucher->prepare($save_query, [":acc_type" => 2,":acc_code" => $bank_code,":debit_acc_head" => $bank_head,
...
":amount" => $amount, ":fin_year"=>$fin_year, ":gjv_serial_no" =>$gjv_serial_no,":dcode" => $dcode,":lbcode" => $lbcode,":statecode" => 33,":getCurrentUser" => $user_name,":getIpAddress" => $ip_address,":edit_id" => $debit_edit_id,":delete_id" => $debit_delete_id], 4);
		...
		$sel_qry_res=$AdjustBankReceiptVoucher->prepare($sel_qry,array( ":dcode"=>$dcode, ":lbcode"=>$lbcode, ":isactive"=>1, ":gjv_chalan_no"=>$gjv_serial_no.'/'.$fin_year, ":account_type"=>2, ":fin_year"=>$fin_year),2);
        ob_start();
        foreach($sel_qry_res as $sel_qry_row){
            ...
            <tr>
                ...
                <td><?php echo htmlentities($sel_qry_row['debit_amount']); ?>
                    <input type="hidden" name="debit_bank_id" value="<?php echo htmlentities($sel_qry_row['gjv_breakupid']);?>" class="bank_id" />
                </td>
                <td>
    ...
    if ($cmd == 4) {
		...
		$sel_qry_res=$AdjustBankReceiptVoucher->prepare($sel_qry,array( ":dcode"=>$dcode, ":lbcode"=>$lbcode, ":isactive"=>1, ":account_type"=>$account_type, ":gjv_breakupid"=>$id),4);
        $Result['STATUS'] = 'SUCCESS';
    ...
    if ($cmd == 6) {
        ...
        if($credit_delete_id == 0 && $credit_edit_id == 0){
            ...
            $res1 = $AdjustBankReceiptVoucher->prepare($save_query, [":acc_type" => 1,":acc_code" => $credit_acc_code,":credit_acc_head" => $credit_acc_head,
		...
		$sel_qry = "select gjv_breakupid,credit_account_id, credit_account_head, credit_amount, b.account_head_name_en,b.account_code from (select gjv_breakupid, credit_account_id, credit_account_head, credit_amount from accounts_master.t_gj_voucher_breakup where dcode=:dcode and lbcode=:lbcode and isactive=:isactive and del_flag is null and gjv_serial_no=:gjv_serial_no  and account_type=:account_type and fin_year=:fin_year)a left join (SELECT account_head_id, old_account_head_code as account_code, account_head_name_en FROM accounts_master.m_account_head)b on a.credit_account_id=b.account_head_id;";		
		$sel_qry_res=$AdjustBankReceiptVoucher->prepare($sel_qry,array( ":dcode"=>$dcode,  ":lbcode"=>$lbcode, ":isactive"=>1, ":gjv_serial_no"=>$gjv_serial_no, ":account_type"=>1, ":fin_year"=>$fin_year),2);
        ob_start();
        foreach($sel_qry_res as $sel_qry_row){
            ?>
            ...
            <tr>
                ...
                <td><?php echo htmlentities($sel_qry_row['credit_amount']); ?>
                    <input type="hidden" name="credit_bank_id" value="<?php echo htmlentities($sel_qry_row['gjv_breakupid']);?>" class="bank_id" />
                </td>
                <td>
    ...
    if ($cmd == 10) {
        ...
        $bpv_check_res = $AdjustBankReceiptVoucher->prepare($bpv_check_query, array(
            ":dcode" => $dcode,
            ":lbcode" => $lbcode,
        ...
        LIMIT 1";
        $sel_dname_res=$AdjustBankReceiptVoucher->prepare($latest_date_query,array(":dcode"=> $dcode,":lbcode"=> $lbcode),4);
        if(isset($sel_dname_res['gjv_date']) && $sel_dname_res['gjv_date']!=''){    
            $latest_date = date('Y-m-d', strtotime($sel_dname_res['gjv_date']));
        }else{
            $query = "SELECT * from public.sp_date_from_fin_year(:fin_year)";
            $query_res=$AdjustBankReceiptVoucher->prepare($query,array(":fin_year"=> $fin_year),4);
