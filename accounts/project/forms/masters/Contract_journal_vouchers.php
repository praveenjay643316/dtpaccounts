...
class ContractJournalVoucher  extends ConfigClass
{
...
$(document).ready(function() {
    ...
    $(document).on('click', '#btn_debit_edit', function() {
        ...
        $.ajax({
            url: "Contract_journal_vouchers.php",
            type: "post",
            data: {
                "account_type": btoa(2),
                "id": btoa(id),
    ...
    $(document).on('change', '#acc_code', function() {
     ...
     let dateStr = '<?php echo date("Y-m-d", strtotime($bpv_date)); ?>';
...
<div class="container mt-3">
    <form action="Contract_journal_vouchers.php" method="post" class="" enctype="multipart/form-data" autocomplete="off">
            ...
            <div class="card-body pl-5 pr-5">
                ...
                <table class="table table-bordered m-0 p-0 table-striped tndtp_form_table">
                    <thead class="bg-th-form-dsg">
                        <tr>
                            <th align="center" scope="col" colspan="12">Contract Journal Voucher</th>
                        </tr>
                    </thead>
                    <tbody>
                        <tr>
                            ...
                            <td scope="col">
                                <?php 
                                        $sel_qry="select max(cjv_serial_no) as cjv_serial_no from accounts_master.t_cj_voucher where dcode=:dcode and lbcode=:lbcode and del_flag is null  and fin_year=:fin_year;";
                                        $sel_qry_res=$this->prepare($sel_qry, array(":dcode"=>$dcode, ":lbcode"=>$lbcode, ":fin_year"=>$fin_year),4);
                                        $serial_no=$sel_qry_res['cjv_serial_no']+1 . '/'.$fin_year;
                                        //$del_qry="delete from accounts_master.t_cj_voucher_breakup where dcode=:dcode and lbcode=:lbcode and fin_year=:fin_year and cjv_chalan_no=:cjv_serial_no and cjv_id is null;";
                                        $del_qry="update accounts_master.t_cj_voucher_breakup set del_flag='Y',                  del_username=:username,del_upd_date=NOW()::timestamp,del_ipaddress=:ipaddress where dcode=:dcode and lbcode=:lbcode and fin_year=:fin_year and cjv_chalan_no=:cjv_serial_no and cjv_id is null;";
                                        $del_qry_res=$this->prepare($del_qry, array(":dcode"=>$dcode, ":lbcode"=>$lbcode, 
                                         ":fin_year"=>$fin_year, ":cjv_serial_no"=> $serial_no),4);
                                        echo  $serial_no;
                        ...
                        <tr>
                            ...
                            <td scope="col">
                                <input type="text" id="date" name="date" value="<?php echo htmlentities(isset($post_data_array['date'])?$post_data_array['date']:'')?>"
                                    class="form-control form-control-sm user_enter_date w-50 date_dd_mm_yyyy" />
                                    
                            </td>
                        </tr>
                     <tr>
    <td class="text-left font-weight-bold">
        <span DisplayLabelID="484">Payment Mode:</span>
    </td>
    <td scope="col">
        <select id="pay_mode" name="pay_mode" class="form-control form-control-sm mb-2 w-50">
            <option value="">choose</option>
            <option value="part-bill">Part-Bill</option>
            <option value="final-bill">Final-Bill</option>
        </select>

        <?php if (isset($post_data_array['pay_mode'])) { ?>
            <script>
                $('#pay_mode').val('<?= $post_data_array['pay_mode'] ?>');
            </script>
        <?php } ?>
    </td>
</tr>
                        ...
                        <tr>
                            <td class="text-left font-weight-bold w-50"><span DisplayLabelID="483">Name Of Contractor</span>
                            </td>
                            <td scope="col">
                                <textarea type="text" id="contractor-name" name="contractor-name"
                                    class="form-control form-control-sm w-50"></textarea>
                                    <?php if (isset($post_data_array['contractor-name'])) { ?>
                                    <script>
                                        $('#contractor-name').val('<?= $post_data_array['contractor-name'] ?>');
                                    </script>
                                <?php } ?>
                            </td>
                        </tr>
                        <tr>
                            <td class="text-left font-weight-bold w-50"><span DisplayLabelID="483">M.Book Reference No</span>
                            </td>
                            <td scope="col">
                                <input type="text" id="mbr_no" name="mbr_no"
                                    class="form-control form-control-sm  w-50" />
                                    <?php if (isset($post_data_array['mbr_no'])) { ?>
                                    <script>
                                        $('#mbr_no').val('<?= $post_data_array['mbr_no'] ?>');
                                    </script>
                                <?php } ?>
                            </td>
                        </tr>
                            ...
                            <tr>
                                ...
                                <td scope="col">
                                    ...
                                    <select id="credit_bank_code" name="credit_bank_code"
                                        class="form-control form-control-sm mb-2">
                                        <option value="">Choose</option>
                                        <?php   $res = $this->Select_Account_Head_Code(2,1);
// $query = "
//                     SELECT 
...
//                         AND li.del_flag IS NULL order by acc_head.account_code asc
//                 ";
//                         $res = $this->prepare($query, [
//                                 ":isactive"=>1,":voucher_id"=>1,":account_type_id"=>1
//                             ], 2);
                        foreach ($res as $row): ?>
                                        <option value="<?= $row['account_head_id']; ?>"
                                            data-code="<?= htmlentities($row['old_code']); ?>"
                    ...
                    <tr>
                        ...
                        <td align="left" colspan="2">
                            <textarea id="narration" name="narration" rows="4" cols="50"
                                class="form-control w-50 form-control-sm"></textarea>
                            
                                 <?php if (isset($post_data_array['narration'])) { ?>
                                    <script>
                                        $('#narration').val('<?= $post_data_array['narration'] ?>');
                                    </script>
                                <?php } ?>
...
<?php
    ...
    public function data_save($save_data)
    {
        ...
        if(isset( $save_data['mbr_no']) &&  $save_data['mbr_no']!=''){
             $mbr_no = $save_data['mbr_no'];
        }else{
             $this->main_content(array_merge(array(
                "STATUS" => "ERROR",
                "STATUS_TYPE" => "FIELD",
        ...
        $res1 = $this->prepare($save_query, array(":dcode" => $dcode, ":lbcode" => $lbcode, ":cjv_chalan_no" => $cjv_serial_no, ":cjv_date" => $date,":pay_mode"=>$pay_mode,":contractor_name"=>$contractor_name,":mbr_no"=>$mbr_no,":name_of_work"=>$name_of_work, ":debit_amount" => $debit_amount,":credit_amount" => $credit_amount,":total_amount" => $credit_amount,":narration" => $narration,":scheme"=>$scheme,":fin_year" => $fin_year,":user_name" => $user_name, ":ip_address" => $ip_address, ":edit_id" => 0, ":del_id" => 0), 4);
        if (!isset($res1->errorInfo)) {
            $inserted_id = $res1['sp_cj_voucher'];
            $latest_date_query = "update accounts_master.t_cj_voucher_breakup set cjv_id=:inserted_id WHERE del_flag IS NULL and cjv_chalan_no=:cjv_serial_no and dcode=:dcode and lbcode=:lbcode and fin_year=:fin_year";
            $sel_dname_res=$this->prepare($latest_date_query,array(":cjv_serial_no"=> $cjv_serial_no,":dcode"=> $dcode,":lbcode"=> $lbcode,":inserted_id"=> $inserted_id, ":fin_year"=>$fin_year),4); 
            $account_head_balance_obj=new Account_head_balance();
            try{
                $account_head_balance_obj->update_voucher_head_amount(1,$cjv_serial_no,False);
                $this->commit();
            }
            catch(Exception $e)
            {
            ...
            $url = $site_data->website_url . '/project/forms/masters/Contractor_Journal_Voucher_Receipt.php?cjvno=' . base64_encode($inserted_id);
            header("Location: $url");
            exit;
        } else {
            $this->rollBack();
...
$ContractJournalVoucher = new ContractJournalVoucher();
if (!isset($_POST['cmd'])) {
    if (isset($_POST['btn_save']) && $_POST['btn_save'] != '') {
        $ContractJournalVoucher->data_save(array_merge($_POST, $_GET));
    } else {
        $ContractJournalVoucher->main_content(array_merge(array("mode_name" => "Save", "mode_class" => "btn-primary"), $_GET));
    }
} else if (isset($_POST['cmd'])) {
    $cmd = base64_decode($_POST['cmd']);
    $dcode = $ContractJournalVoucher->getCurrentDistrictCode();
        $lbcode = $ContractJournalVoucher->getCurrentLocalBodyCode(); 
    if ($cmd == 2) {
		...
		$sel_qry_res=$ContractJournalVoucher->prepare($sel_qry,array(":bank_code"=>$bank_code, ":dcode"=>$dcode, ":lbcode"=>$lbcode, ":isactive"=>1),4);
    ...
    if ($cmd == 3) {
        ...
        $dcode = $ContractJournalVoucher->getCurrentDistrictCode();
        $lbcode = $ContractJournalVoucher->getCurrentLocalBodyCode();
        $user_name = $ContractJournalVoucher->getCurrentUser();
        $ip_address = $ContractJournalVoucher->getIpAddress();
        $fin_year = $ContractJournalVoucher->getFinYear();
        $ContractJournalVoucher->beginTransaction();
        if($debit_delete_id == 0 && $debit_edit_id == 0){
            ...
            $res1 = $ContractJournalVoucher->prepare($save_query, [":acc_type" => 2,":acc_code" => $bank_code,":debit_acc_head" => $bank_head,
...
":amount" => $amount, ":fin_year"=>$fin_year, ":cjv_serial_no" =>$cjv_serial_no,":dcode" => $dcode,":lbcode" => $lbcode,":statecode" => 33,":getCurrentUser" => $user_name,":getIpAddress" => $ip_address,":edit_id" => $debit_edit_id,":delete_id" => $debit_delete_id], 4);
        ...
        if(!isset($res1->errorInfo)){
            $sel_qry = "select cjv_breakupid,debit_account_id, debit_account_head, debit_amount, b.account_code ,b.account_head_name_en from (select cjv_breakupid, debit_account_id, debit_account_head, debit_amount from accounts_master.t_cj_voucher_breakup where dcode=:dcode and lbcode=:lbcode and isactive=:isactive and del_flag is null and cjv_chalan_no=:cjv_serial_no  and account_type=:account_type and fin_year=:fin_year)a left join (SELECT account_head_id, old_account_head_code as account_code, account_head_name_en FROM accounts_master.m_account_head)b on a.debit_account_id=b.account_head_id;";		
            $sel_qry_res=$ContractJournalVoucher->prepare($sel_qry,array( ":dcode"=>$dcode, ":lbcode"=>$lbcode, ":isactive"=>1, ":cjv_serial_no"=>$cjv_serial_no, ":account_type"=>2, ":fin_year"=>$fin_year),2);
            ob_start();
    ...
    if ($cmd == 4) {
		...
		$sel_qry_res=$ContractJournalVoucher->prepare($sel_qry,array( ":dcode"=>$dcode, ":lbcode"=>$lbcode, ":isactive"=>1, ":account_type"=>$account_type, ":cjv_breakupid"=>$id),4);
        $Result['STATUS'] = 'SUCCESS';
    ...
    if ($cmd == 6) {
        ...
        if($credit_delete_id == 0 && $credit_edit_id == 0){
            ...
            $res1 = $ContractJournalVoucher->prepare($save_query, [":acc_type" => 1,":acc_code" => $credit_acc_code,":credit_acc_head" => $credit_acc_head,
		...
		$sel_qry = "select cjv_breakupid,credit_account_id, credit_account_head, b.account_code , credit_amount, b.account_head_name_en from (select cjv_breakupid, credit_account_id, credit_account_head, credit_amount from accounts_master.t_cj_voucher_breakup where dcode=:dcode and lbcode=:lbcode and isactive=:isactive and del_flag is null and cjv_chalan_no=:cjv_serial_no  and account_type=:account_type and fin_year=:fin_year)a left join (SELECT account_head_id, old_account_head_code as account_code, account_head_name_en FROM accounts_master.m_account_head)b on a.credit_account_id=b.account_head_id;";		
		$sel_qry_res=$ContractJournalVoucher->prepare($sel_qry,array( ":dcode"=>$dcode,  ":lbcode"=>$lbcode, ":isactive"=>1, ":cjv_serial_no"=>$cjv_serial_no, ":account_type"=>1, ":fin_year"=>$fin_year),2);
        ob_start();
        foreach($sel_qry_res as $sel_qry_row){
            ?>
        <tr>
    ...
    if ($cmd == 10) {
        ...
        $bpv_check_res = $ContractJournalVoucher->prepare($bpv_check_query, array(
            ":dcode" => $dcode,
            ":lbcode" => $lbcode,
    ...
    {   
        ...
        $lang_code_2d = $ContractJournalVoucher->getCurrentUserLanguage2D();
        $query="SELECT 
                cwd.scheme_id,
            ...
            WHERE 
                ...
                AND cwd.dcode = :dcode;
            ";
        $res=$ContractJournalVoucher->prepare($query,[":cwd_id"=>$cwd_id,":lbcode"=>$lbcode,":dcode"=>$dcode],4);
