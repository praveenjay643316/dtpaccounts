...
if($_SERVER['REQUEST_METHOD']=='GET')
{
    ...
    if( !isset($_GET['id']))
    {
        ...
        $pjv_id=base64_decode($_GET['id']);
        $res=$conn->prepare('select count(*) as "count" from accounts_master.t_pj_voucher where pjv_id=:id and dcode=:dcode and lbcode=:lbcode and fin_year=:fin_year and del_flag is null',[":id"=>$pjv_id,":dcode"=>$dcode,":lbcode"=>$lbcode,":fin_year"=>$fin_year],4);
        if($res["count"]==0)
        {
            echo("Voucher Number Does not exist");
            die();
        }
        $voucher_res=$conn->prepare('select pjv_chalan_no from accounts_master.t_pj_voucher where pjv_id=:pjv_id and dcode=:dcode and lbcode=:lbcode and fin_year=:fin_year and del_flag is null',[":dcode"=>$dcode,":lbcode"=>$lbcode,":fin_year"=>$fin_year,":pjv_id"=>$pjv_id],4);
        $voucher_no=$voucher_res['pjv_chalan_no'];
    }
}

class EditPurchaseJournalVouchers  extends ConfigClass
{

    public $page_token = "Purchase_Journal_Vouchers";
...
<?php
       ...
       $pjv_id = base64_decode($_GET['id']);
        $voucher_res=$this->prepare('select pjv_chalan_no from accounts_master.t_pj_voucher where pjv_id=:pjv_id and dcode=:dcode and lbcode=:lbcode and fin_year=:fin_year and del_flag is null',[":dcode"=>$dcode,":lbcode"=>$lbcode,":fin_year"=>$fin_year,":pjv_id"=>$pjv_id],4);
        $voucher_no=$voucher_res['pjv_chalan_no'];
...
$(document).ready(function() {
        ...
        $.ajax({
            ...
            success: function(data) {
                if (data != '') {
                    ...
                    $('#debit_delete_id').val('');
                    $('#debit_edit_id').val(Result_Data['pjv_breakupid']);
                }
            },
            dataType: 'html'
        });
        ...
        $.ajax({
            ...
            success: function(data) {
                if (data != '') {
                    ...
                    $('#debit_bank_head').val(Result_Data['bank_head']);
                    $('#debit_amount').val(Result_Data['debit_amount']);
                    $('#debit_delete_id').val(Result_Data['pjv_breakupid']);
                    $('#debit_edit_id').val('');
                }
            },
            dataType: 'html'
        ...
        $.ajax({
            ...
            success: function(data) {
                if (data != '') {
                    ...
                    $('#credit_edit_id').val(Result_Data['pjv_breakupid']);
                }
            },
            dataType: 'html'
        });
        ...
        $.ajax({
            ...
            success: function(data) {
                if (data != '') {
                    ...
                    $('#credit_amount').val(Result_Data['credit_amount']);
                    $('#credit_delete_id').val(Result_Data['pjv_breakupid']);
                    $('#credit_edit_id').val('');
                }
            },
            dataType: 'html'
    ...
    $(document).on('click', '#btn_debit_add', function() {
        try {
            if ($("#pjv_serial_no").val().length == '') {
                throw {
                    msg: "Missing Serial Number",
                    foc: "#pjv_serial_no"
                }
            } else {
                var pjv_serial_no = $("#pjv_serial_no").val();
            }
        ...
        $.ajax({
            ...
            data: {
                ...
                "pjv_serial_no": btoa(pjv_serial_no),
    ...
    function intialSetUp() {
        ...
        $.ajax({
            ...
            success: function(data) {
                //console.log(data)
                //pjv_date
                let curr_date = data.pjv_date.split(" ")[0].split("-");
                let formatted_date = curr_date[2] + "-" + curr_date[1] + "-" + curr_date[0];
                $(".pjv_date").val(formatted_date);
            ...
            <div class="card-body pl-5 pr-5">
                ...
                <table class="table table-bordered m-0 p-0 table-striped tndtp_form_table">
                    <thead class="bg-th-form-dsg">
                        <tr>
                            <th align="center" scope="col" colspan="12">Edit Purchase Journal Voucher</th>
                        </tr>
                    </thead>
                    <tbody>
                        <tr>
                            ...
                            <td scope="col">
                                <?php   
                                        //$query_voucher_no=base64_decode($_GET['query_voucher_no']);
                                            ...
                                            /*
                                        $sel_qry="select max(pjv_no) as id from accounts_master.t_pj_voucher where dcode=:dcode and lbcode=:lbcode and del_flag is null  and fin_year=:fin_year;";
...
;
                                        ...
                                        ?>
                                <input type="hidden" id="pjv_serial_no" name="pjv_serial_no"
                                    class="form-control w-50 form-control-sm"
                                    value="<?php echo $chalan_no_parts[0]; ?>" />
                                    <input type="hidden" id="pjv_chalan_no" name="pjv_chalan_no"
                                    class="form-control w-50 form-control-sm"
                                    value="<?php echo $voucher_no; ?>" />
                            </td>
                        </tr>
                        <tr>
                            <td class="text-left font-weight-bold"><span DisplayLabelID="484">Pjv Date</span></td>
                            <td scope="col">
                                <input type="text" id="date" name="date" value=""
                                    class="form-control form-control-sm pjv_date user_enter_date w-50" disabled/>
                                                                        <input type="hidden" class="pjv_date" name="date" value=""/>
                    ...
                    <tr>
                        <td colspan="4" align="center">
                            ...
                            <input type="button" id="btn_reset" name="btn_reset" value="Cancel"
                                ...
                                onclick="window.location='Purchase_Journal_Vouchers.php'" />
                        </td>
                    </tr>
                    </tbody>
                </table>
...
// print_r($save_data);
        ...
        if (isset($save_data['pjv_serial_no']) && $save_data['pjv_serial_no']!='') {
            $pjv_serial_no = $save_data['pjv_serial_no'];
            $pjv_serial_no_Validation = $this->Field_Validation(
                array(
                    'Field_Type' => 'number',
                    'Field_Value' => $save_data['pjv_serial_no'],
                    'Field_Name' => 'BRV Chalan Number',
                    'Field_Label_Name' => 'BRV Chalan Number',
                )
            );

            if ($pjv_serial_no_Validation['Status'] == "Error") {
                $this->main_content(array_merge(array(
                    "STATUS" => "ERROR",
                    "STATUS_TYPE" => "FIELD",
                    "FIELD_NAME" => "pjv_serial_no",
                    "MESSAGE" => $pjv_serial_no_Validation['Message']
                ), $save_data));
                exit;
            }
        }else{
        ...
        $sp_bank_receicpt_voucher = "accounts_master.sp_pj_voucher";
        $user_name = $this->getCurrentUser();
        $ip_address = $this->getIpAddress();       
        $save_query = "select * from " . $sp_bank_receicpt_voucher . "(:dcode, :lbcode, :pjv_no, :pjv_date,:debit_amount, :credit_amount, :total_amount, :narration, :fin_year, :user_name, :ip_address, :edit_id, :del_id)";
        $res1 = $this->prepare($save_query, array(":dcode" => $dcode, ":lbcode" => $lbcode, ":pjv_no" => $pjv_serial_no, ":pjv_date" => $date, ":fin_year" => $fin_year, ":debit_amount" => $debit_amount, ":credit_amount" => $credit_amount, ":total_amount" => $credit_amount, ":narration" => $narration, ":user_name" => $user_name, ":ip_address" => $ip_address, ":edit_id" => 1, ":del_id" => 0), 4);
        //print_r($res1);
        //die();
        if (!isset($res1->errorInfo)) {


            $inserted_id = $res1['sp_pj_voucher'];

            $latest_date_query = "update accounts_master.t_pj_voucher_breakup set voucher_no=:inserted_id   WHERE del_flag IS NULL and pjv_serial_no=:pjv_serial_no and dcode=:dcode and lbcode=:lbcode";
            $sel_dname_res=$this->prepare($latest_date_query,array(":pjv_serial_no"=> $pjv_serial_no,":dcode"=> $dcode,":lbcode"=> $lbcode,":inserted_id"=> $inserted_id),4); 
...
$EditPurchaseJournalVouchers = new EditPurchaseJournalVouchers();
$pjv_id = base64_decode($_GET['id']);
$lbcode=$EditPurchaseJournalVouchers->getCurrentLocalBodyCode();
$dcode=$EditPurchaseJournalVouchers->getCurrentDistrictCode();
$fin_year=$EditPurchaseJournalVouchers->getFinYear();
$voucher_res=$EditPurchaseJournalVouchers->prepare('select pjv_chalan_no from accounts_master.t_pj_voucher where pjv_id=:pjv_id and dcode=:dcode and lbcode=:lbcode and fin_year=:fin_year and del_flag is null',[":dcode"=>$dcode,":lbcode"=>$lbcode,":fin_year"=>$fin_year,":pjv_id"=>$pjv_id],4);
        $voucher_no=$voucher_res['pjv_chalan_no'];
if (!isset($_POST['cmd'])) {
    if (isset($_POST['btn_save']) && $_POST['btn_save'] != '') {
        $EditPurchaseJournalVouchers->data_save(array_merge($_POST, $_GET));
    } else {
        $EditPurchaseJournalVouchers->main_content(array_merge(array("mode_name" => "Save", "mode_class" => "btn-primary"), $_GET));
    }
    ...
    if ($cmd == 2) {
		...
		$sel_qry_res=$EditPurchaseJournalVouchers->prepare($sel_qry,array(":bank_code"=>$bank_code, ":dcode"=>$dcode, ":lbcode"=>$lbcode, ":isactive"=>1),4);
    ...
    if ($cmd == 3) {
        ...
        $pjv_serial_no = base64_decode($_POST['pjv_serial_no']);
        $bank_code = base64_decode($_POST['bank_code']);
        ...
        $dcode = $EditPurchaseJournalVouchers->getCurrentDistrictCode();
        $lbcode = $EditPurchaseJournalVouchers->getCurrentLocalBodyCode();
        $user_name = $EditPurchaseJournalVouchers->getCurrentUser();
        $ip_address = $EditPurchaseJournalVouchers->getIpAddress();
        $fin_year = $EditPurchaseJournalVouchers->getFinYear();
        //$fin_year='2024-2025';
        $EditPurchaseJournalVouchers->beginTransaction();
        if($debit_delete_id == 0 && $debit_edit_id == 0){
            ...
            // $res1 = $EditPurchaseJournalVouchers->prepare($save_query, array(":dcode" => $dcode, ":lbcode" => $lbcode, ":account_type" => 1, ":bank_code" => $bank_code, ":bank_head" => $bank_head, ":debit_amount" => $amount, ":credit_amount"=>NULL, ":user_name" => $user_name, ":ip_address" => $ip_address, ":edit_id" => $debit_edit_id, ":del_id" => $debit_delete_id, ":fin_year"=>$fin_year, ":pjv_serial_no" =>$pjv_serial_no),4);
        ...
        $res1 = $EditPurchaseJournalVouchers->prepare($save_query, [":acc_type" => 2,":acc_code" => $bank_code,":debit_acc_head" => $bank_head,
":amount" => $amount, ":fin_year"=>$fin_year, ":pjv_serial_no" =>$pjv_serial_no,":dcode" => $dcode,":lbcode" => $lbcode,":statecode" => 33,":getCurrentUser" => $user_name,":getIpAddress" => $ip_address,":edit_id" => $debit_edit_id,":delete_id" => $debit_delete_id], 4);
...
":amount" => $amount, ":fin_year"=>$fin_year, ":pjv_serial_no" =>$pjv_serial_no,":dcode" => $dcode,":lbcode" => $lbcode,":statecode" => 33,":getCurrentUser" => $user_name,":getIpAddress" => $ip_address,":edit_id" => $debit_edit_id,":delete_id" => $debit_delete_id], 4);
		...
		$sel_qry = "select pjv_breakupid,debit_account_id, debit_account_head, debit_amount, b.account_head_name_en,b.account_code from (select pjv_breakupid, debit_account_id, debit_account_head, debit_amount from accounts_master.t_pj_voucher_breakup where dcode=:dcode and lbcode=:lbcode and isactive=:isactive and del_flag is null and pjv_serial_no=:pjv_serial_no  and account_type=:account_type and fin_year=:fin_year)a left join (SELECT account_head_id, old_account_head_code as account_code, account_head_name_en FROM accounts_master.m_account_head)b on a.debit_account_id=b.account_head_id;";		
		$sel_qry_res=$EditPurchaseJournalVouchers->prepare($sel_qry,array( ":dcode"=>$dcode, ":lbcode"=>$lbcode, ":isactive"=>1, ":pjv_serial_no"=>(int)$pjv_serial_no, ":account_type"=>2, ":fin_year"=>$fin_year));
        ob_start();
...
<tr>
    ...
    <td><?php echo htmlentities($sel_qry_row['debit_amount']); ?>
        <input type="hidden" name="debit_bank_id" value="<?php echo htmlentities($sel_qry_row['pjv_breakupid']);?>"
            class="bank_id" />
    </td>
    <td>
...
<?php
         ...
         $debit_amount = array_sum(array_column($sel_qry_res, 'debit_amount'));
		    ...
		    ob_clean();
            $EditPurchaseJournalVouchers->commit();
    ...
    if ($cmd == 4) {
		...
		$sel_qry_res=$EditPurchaseJournalVouchers->prepare($sel_qry,array( ":dcode"=>$dcode, ":lbcode"=>$lbcode, ":isactive"=>1, ":account_type"=>$account_type, ":pjv_breakupid"=>$id),4);
        $Result['STATUS'] = 'SUCCESS';
    ...
    if ($cmd == 8) {
        ...
        $dcode = $EditPurchaseJournalVouchers->getCurrentDistrictCode();
        $lbcode = $EditPurchaseJournalVouchers->getCurrentLocalBodyCode();
        $sel_qry = "select pjv_breakupid, credit_account_id, credit_account_head, credit_amount from accounts_master.t_pj_voucher_breakup where dcode=:dcode and lbcode=:lbcode and isactive=:isactive and del_flag is null and account_type=:account_type and pjv_breakupid=:pjv_breakupid;";      
        $sel_qry_res=$EditPurchaseJournalVouchers->prepare($sel_qry,array( ":dcode"=>$dcode, ":lbcode"=>$lbcode, ":isactive"=>1, ":account_type"=>$account_type, ":pjv_breakupid"=>$id),4);
        $Result['STATUS'] = 'SUCCESS';
    ...
    if ($cmd == 6) {
        ...
        if($credit_delete_id == 0 && $credit_edit_id == 0){
            ...
            $save_query = "SELECT accounts_master.sp_pj_voucher_breakup(:acc_type,:acc_code,:credit_acc_head,:amount, :fin_year, :pjv_serial_no,:dcode ,:lbcode,:statecode,:getCurrentUser,:getIpAddress,:edit_id,:delete_id);";
            $res1 = $EditPurchaseJournalVouchers->prepare($save_query, [":acc_type" => 1,":acc_code" => $credit_acc_code,":credit_acc_head" => $credit_acc_head,
                ":amount" => $amount, ":fin_year"=>$fin_year, ":pjv_serial_no" =>$pjv_serial_no,":dcode" => $dcode,":lbcode" => $lbcode,":statecode" => 33,":getCurrentUser" => $user_name,":getIpAddress" => $ip_address,":edit_id" => $credit_edit_id,":delete_id" => $credit_delete_id], 4);
		...
		$sel_qry = "select pjv_breakupid,credit_account_id, credit_account_head, credit_amount, b.account_head_name_en from (select pjv_breakupid, credit_account_id, credit_account_head, credit_amount from accounts_master.t_pj_voucher_breakup where dcode=:dcode and lbcode=:lbcode and isactive=:isactive and del_flag is null and pjv_serial_no=:pjv_serial_no  and account_type=:account_type and fin_year=:fin_year)a left join (SELECT account_head_id, old_account_head_code as account_code, account_head_name_en FROM accounts_master.m_account_head)b on a.credit_account_id=b.account_head_id;";		
		$sel_qry_res=$EditPurchaseJournalVouchers->prepare($sel_qry,array( ":dcode"=>$dcode,  ":lbcode"=>$lbcode, ":isactive"=>1, ":pjv_serial_no"=>$pjv_serial_no, ":account_type"=>1, ":fin_year"=>$fin_year),2);
        ob_start();
        foreach($sel_qry_res as $sel_qry_row){
            ?>
...
<tr>
    ...
    <td><?php echo htmlentities($sel_qry_row['credit_amount']); ?>
        <input type="hidden" name="credit_bank_id" value="<?php echo htmlentities($sel_qry_row['pjv_breakupid']);?>"
            class="bank_id" />
    </td>
    <td>
...
<?php
    ...
    if ($cmd == 7) {
        ...
        $res1 = $EditPurchaseJournalVouchers->prepare($save_query, array(":dcode" => $dcode, ":lbcode" => $lbcode, ":chalan_date" => $chl_date, ":isactive" => 1),4);
    ...
    */
    if ($cmd == 10) {
        ...
        list($date_dateofreceived,$month_dateofreceived,$year_dateofreceived)=explode('-',base64_decode($_POST['pjv_date']));
       ...
       $bpv_check_query = "SELECT COUNT(*) as cnt FROM accounts_master.t_bank_payment_voucher 
        ...
        $bpv_check_res = $EditPurchaseJournalVouchers->prepare($bpv_check_query, array(
            ":dcode" => $dcode,
            ":lbcode" => $lbcode,
        ...
        ?>
<?php 
        $latest_date_query = "SELECT pjv_date 
        FROM accounts_master.t_pj_voucher
        WHERE del_flag IS NULL  and dcode=:dcode and lbcode=:lbcode
        ORDER BY pjv_date DESC 
        LIMIT 1";
        $sel_dname_res=$EditPurchaseJournalVouchers->prepare($latest_date_query,array(":dcode"=> $dcode,":lbcode"=> $lbcode),4);
        //print_r($sel_dname_res);die();
         $chalan_date_raw = base64_decode($_POST['pjv_date']); 

            $chalan_date = DateTime::createFromFormat('d-m-Y', $chalan_date_raw)->format('Y-m-d');

            $latest_date = date('Y-m-d', strtotime($sel_dname_res['pjv_date']));
        ...
        if($cmd==11)
        {
            ...
            $query="select * from accounts_master.t_pj_voucher where pjv_chalan_no=:chalan_no and del_flag is null and lbcode=:lbcode and dcode=:dcode and fin_year=:fin_year";
            $params=[":chalan_no"=>$voucher_no,":lbcode"=> $lbcode,
                    ":dcode"=>$dcode,":fin_year"=>$fin_year];
            $res=$EditPurchaseJournalVouchers->prepare($query,$params,4);
        ...
        {   
           ...
           $query="select a.*,b.account_code from accounts_master.t_pj_voucher_breakup as a left join (SELECT account_head_id, old_account_head_code as account_code, account_head_name_en FROM accounts_master.m_account_head)b on a.debit_account_id=b.account_head_id where credit_amount is null and pjv_id=:pjv_id and del_flag is null and lbcode=:lbcode and dcode=:dcode and fin_year=:fin_year;";


            $params=[":pjv_id"=>$voucher_id,":lbcode"=>$lbcode,":dcode"=>$dcode,":fin_year"=>$fin_year];

            $res=$EditPurchaseJournalVouchers->prepare($query,$params,2);
            $html="";
            foreach($res as $sel_qry_row)
            {
                ...
                "<td>{$sel_qry_row['debit_amount']}
                    <input type='hidden' name='debit_bank_id' value='{$sel_qry_row['pjv_breakupid']}' class='bank_id' />
                </td>"
                . "<td>
...
$query="select a.*,b.account_code from accounts_master.t_pj_voucher_breakup  as a left join (SELECT account_head_id, old_account_head_code as account_code, account_head_name_en FROM accounts_master.m_account_head)b on a.credit_account_id=b.account_head_id where debit_amount is null and pjv_id=:pjv_id and del_flag is null and lbcode=:lbcode and dcode=:dcode and fin_year=:fin_year;";
...
$curr="<tr>
    ...
    <td>" . htmlentities($sel_qry_row['credit_amount']) . "
        <input type=\"hidden\" name=\"credit_bank_id\" value=\"" . htmlentities($sel_qry_row['pjv_breakupid']) . "\"
            class=\"bank_id\" />
    </td>
    <td>
        <input type=\"button\" id=\"btn_credit_edit\" name=\"btn_credit_edit\" value=\"Edit\" class=\"btn btn-md
            text-white font-weight-bold btn-success\" style=\"font-size: small;\">
        
    </td>
</tr>";
/*
<input type=\"button\" id=\"btn_credit_delete\" name=\"btn_credit_delete\" value=\"Delete\" class=\"btn btn-md
            text-white font-weight-bold btn-danger\" style=\"font-size: small;\">
 */
$html.=$curr;
