    ...
    public function main_content($post_data_array = array())
    {
        ...
        <script type="text/javascript">
            ...
            $('#amount').on('blur', function () {
                    $('#amount_hidden').val($(this).val());
                });
                    $('#cash_coll_date_row').hide();

                    $('#cash_coll_bill_collector').hide();
                $('input[name="cash_from_type"]').on('change', function () {
               ...
               var today = new Date();
                ...
                $('#cash_coll_date').datepicker({
                 ...
                 $('#bill_collector_name').change(function () {
                    const pay_mode = $("#pay_mode").val();
                    const collection_date = $('#cash_coll_date').val();
                    const bill_collector_id = $('#bill_collector_name').val();
                    if (pay_mode === "") {
                        $(this).val("");
                        alert("Please select Payment Mode.");
                        return false;
                    }
                    if (bill_collector_id === "") {
                        $(this).val("");
                        alert("Please select Bill Collector Name.");
                        return false;
                    }
                    if (collection_date === "") {
                        $(this).val("");
                            ...
                            if (response.STATUS == 'FAILED') {
                                ...
                                $.ajax({
                                    ...
                                    data: {
                                        ...
                                        "cmd": btoa(2),
                                        "collection_date": btoa(collection_date),
                                        "bill_collector_id": btoa(bill_collector_id),
                                    },
                                    success: function (response) {
                                        try {
                                            ...
                                            if (data.grand_total !== undefined) {
                                                ...
                                                $('#coll_amount_hidden').val(data.grand_total);
                                            }
                                        } catch (e) {
                ...
                var today = new Date();
                today.setHours(0, 0, 0, 0);  // Reset time to midnight (start of today)
                ...
                */
                ...
                $(document).on('click', '#btn_debit_add', function() {
                    ...
                    var debit_amount = parseFloat($("#debit_amount").val()) || 0; 
                    // Check for zero
                    if (debit_amount === 0) {
                        alert('0 is not allowed. Please enter a valid amount.');
                        return false;
                    }
                    ...
                    if (delete_id === 0) {
                    let totalAmount    = Number($('#amount_hidden').val());
                    let existingDebit  = Number($('#debit_total_amount').val()) || 0;
                    let debitAmount    = debit_amount;
                    let editId         = Number($('#debit_edit_id').val()) || 0;
                    let oldEditAmount  = Number($('#debit_edit_old_amount').val()) || 0;

                    let newDebit;

                    if (editId > 0) {
                        newDebit = existingDebit - oldEditAmount + debitAmount;
                    } else {
                        newDebit = existingDebit + debitAmount;
                    }



                    if (newDebit > totalAmount) {
                        alert('Debit amount cannot exceed Chalan Amount (' + totalAmount + ')');
                        return false;
                    }
                }
						...
						success: function(data) {
							if (data != '') {
                                ...
                                alert(message);
                                    ...
                                    $('#debit_total_amount').val(Result_Data['debit_amount']);
                                    debit_total_amount=Result_Data['debit_amount'];
                                    $('#debit_total_amount').val(Result_Data['debit_amount']);
                                    debit_total_amount=Result_Data['debit_amount'];
                                    ...
                                    if(paymode==1)
                                    {
                                        ...
                                        let row=`<tr><td>${3059}</td><td>3059 - General Account <td>${debit_total_amount}</td></tr>`;
                                        if(debit_total_amount<=0)
                                        {
                                                                                  $("#credit_table_result tbody").html('');
                                                                                  $('#credit_total_amount').val('');
                                        $('#span_credit_total_amount').html('');
  
                                        }
                                        else{
                                            $("#credit_table_result tbody").html(row);
                                        //$("#debit_bank_code").val('2');
                                        $('#credit_total_amount').val(debit_total_amount);
                                        $('#span_credit_total_amount').html(debit_total_amount);
                                        }
                ...
                $(document).on('click', '#btn_credit_add', function(){
                   var credit_amount = parseFloat($("#credit_amount").val()) || 0; 
                    // Check for zero
                    if (credit_amount === 0) {
                        alert('0 is not allowed. Please enter a valid amount.');
                        return false;
...
.test($("#credit_amount").val().trim())) {
                    ...
                    if (delete_id === 0) {
                    let totalAmount      = Number($('#amount_hidden').val());
                    let existingCredit   = Number($('#credit_total_amount').val()) || 0;
                    let creditAmount     = Number(credit_amount);
                    let editId           = Number($('#credit_edit_id').val()) || 0;
                    let oldEditAmount    = Number($('#credit_edit_old_amount').val()) || 0;

                    let newCredit;

                    if (editId > 0) {
                        // EDIT MODE
                        newCredit = existingCredit - oldEditAmount + creditAmount;
                    } else {
                        // ADD MODE
                        newCredit = existingCredit + creditAmount;
                    }

                    

                    if (newCredit > totalAmount) {
                        alert(
                            'Credit amount cannot exceed Chalan Amount (' + totalAmount + ')'
                        );
                        return false;
                    }
                }
						...
						success: function(data) {
							if (data != '') {
                                ...
                                if(Result_Data['STATUS'] == 'SUCCESS'){
                                    ...
                                    $('#span_credit_total_amount').html(Result_Data['credit_amount']!=0?Result_Data['credit_amount']:'');
                                    $('#credit_delete_id').val('');
                                    $('#credit_edit_id').val('');
                    ...
                    $.ajax({
						...
						success: function(data) {
							if (data != '') {
                                ...
                                $('#debit_edit_old_amount').val(Result_Data['debit_amount']);
                    ...
                    $.ajax({
						...
						success: function(data) {
							if (data != '') {
                                ...
                                $('#credit_edit_old_amount').val(Result_Data['credit_amount']);
                                $('#credit_delete_id').val('');
                ...
                var payModeText = $('#pay_mode option:selected').text();
                var payModeVal  = $(this).val();

                // -------- RESET CREDIT (FIX) ----------
                $('#credit_bank_code').prop('disabled', false).val('');
                $('#credit_bank_code option[value="865"]').show();
                $("#btn_credit_add").show();
                $("#credit_amount").show();
                $("#credit_current_amount_tr").show();

                // -------- LABELS ----------
                if (payModeText !== 'Choose') {
                    $('#pay_mode_label').text(payModeText + ' From');
                    $('#pay_mode_label_date').text(payModeText + ' Collection Date');
                } else {
                    $('#pay_mode_label').text('Cash From');
                    $('#pay_mode_label_date').text('Cash Collection Date');
                }

                // -------- DEBIT COMMON ----------
                $("#debit_current_amount_tr").show();
                $('#debit_bank_code').prop('disabled', false).val('');
                $('#debit_bank_code option[value="865"]').hide();
                $('#debit_amount').show();
                $('#bank_code_row').show();
                $("#btn_debit_add").show();

                // -------- PAY MODE CONDITIONS ----------
                if (payModeVal == '2') { // CHEQUE
                    $('.pay_mode_dd, .pay_mode_ecs').hide();
                    $('.pay_mode_cheque').show();
                    $('#bank_name_row, #bank_code_row, #bank_branch_row').show();

                } else if (payModeVal == '3') { // DD
                    $('.pay_mode_cheque, .pay_mode_ecs').hide();
                    $('.pay_mode_dd').show();
                    $('#bank_name_row, #bank_code_row').show();
                    $('#bank_branch_row').hide();

                } else if (payModeVal == '4') { // ECS
                    $('.pay_mode_dd, .pay_mode_cheque').hide();
                    $('.pay_mode_ecs').show();
                    $('#bank_name_row, #bank_code_row, #bank_branch_row').hide();

                } else { // CASH
                    $('.pay_mode_dd, .pay_mode_cheque, .pay_mode_ecs').hide();
                    $('#bank_name_row, #bank_branch_row, #bank_code_row').hide();

                    $("#btn_credit_add").hide();
                    $("#credit_amount").hide();
                    $("#credit_current_amount_tr").hide();

                    $('#credit_bank_code').val(865);   // cash head
                    $('#credit_bank_code').prop('disabled', true);
                }

                $('#current_pay_mode').val(payModeVal);
            });
                ...
                $(document).on('click', "#btn_save", function () {
                    ...
                    try {
                        ...
                        if (selectedType === 'Collection') {
                            ...
                            enteredAmount = parseFloat($("#acc_amount_hidden").val() || 0);
                        }

                        let totalAmount = parseFloat($("#amount_total").val() || 0);
                ...
                $(document).on('click', "#add_amount", function () {
                    ...
                    $.ajax({
                        url: 'Adjust_Triplicate_Chalan.php', // your PHP file
                        type: 'POST',
                        dataType: "html",
                        data: {
...
document.getElementById('credit_bank_code').addEventListener('change', function() {
    ...
    document.getElementById('credit_bank_head').value = accountName;
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        $sel_qry = "select max(tc_serial_no::integer) as tc_serial_no from accounts_master.t_triplicate_chalan_details where dcode=:dcode and fin_year=:fin_year and lbcode=:lbcode and isactive=:isactive and del_flag is null;";
                                        //$sel_qry="select chalan_details_id as id , chalan_no from accounts_master.t_triplicate_chalan_details where dcode=:dcode and lbcode=:lbcode and isactive=:isactive and del_flag is null order by chalan_details_id desc;";
                                        $sel_qry_res = $this->prepare($sel_qry, array(":dcode" => $dcode, ":lbcode" => $lbcode, ":isactive" => 1,":fin_year" => $fin_year), 4);
                                        
                                        //print_r($fin_year);
                                        $del_qry = "delete from accounts_master.t_triplicate_accounthead_breakup where dcode=:dcode and lbcode=:lbcode and fin_year=:fin_year and tc_serial_no=:chalan_no and challan_id is null;";
                                        $del_qry_res = $this->prepare($del_qry, array(":dcode" => $dcode, ":lbcode" => $lbcode, ":fin_year" => $fin_year, ":chalan_no" => $sel_qry_res['tc_serial_no'] + 1), 4);
                                        //chalan_no is same as chalan_details_id applicable only for triplicate chalan
                                        echo $sel_qry_res['tc_serial_no'] + 1 . '/' . $fin_year ;
                                        ?>
                                        <input type="hidden" id="rc_serial_no" name="rc_serial_no"
                                            value="<?php echo $sel_qry_res['tc_serial_no'] + 1; ?>"
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <select id="pay_mode" name="pay_mode" class="form-control form-control-sm w-50">
                                            ...
                                            <?php
                                            $sel_payment_type = "select paymenttypeid, paymenttype as paymenttype_en, paymenttype_ta from master.m_paymenttype where del_flag is null and paymenttypeid  in (1,4,2,3);";
                                        ...
                                        <input type="hidden" id="current_pay_mode" name="current_pay_mode" />
                                        <script type="text/javascript">
                                ...
                                <tr class="pay_mode_dd" style="display:none;">
                                    <td class="text-left font-weight-bold"><span>Remitter DD No</span></td>
                                    <td><input type="text" id="dd_no" name="dd_no" class="form-control form-control-sm w-50" />
                                    </td>
                                </tr>
                                <tr class="pay_mode_dd" style="display:none;">
                                    <td class="text-left font-weight-bold"><span>Remitter DD Date</span></td>
                                ...
                                <tr id="bank_code_row" style="display:none;">
                                    <td class="text-left font-weight-bold"><span>Remitter Bank Name</span></td>
                                    <td>
                                        <select id="bank_code" name="bank_code" class="form-control form-control-sm w-50">
                                            ...
                                            foreach ($sel_bank_newid_res as $sel_bank_newid_row) {
                                                ...
                                                <option value="<?php echo htmlentities($sel_bank_newid_row['bank_id']); ?>"
                                                    ...
                                                    <?php echo htmlentities($sel_bank_newid_row['bank_name_' . $lang_code_2d]); ?>
                                                </option>
                                            <?php } ?>
                                        </select>
                                        <script type="text/javascript">
                                ...
                                <tr id="bank_branch_row" style="display:none;">
                                    <td class="text-left font-weight-bold"><span>Remitter Bank Branch</span></td>
                                ...
                                <tr class="pay_mode_ecs" style="display:none;">
                                    <td class="text-left font-weight-bold"><span>Remitter ECS No</span></td>
                                    <td><input type="text" id="ecs_no" name="ecs_no"
                                            class="form-control form-control-sm w-50" /></td>
                                </tr>
                                <tr class="pay_mode_ecs" style="display:none;">
                                    <td class="text-left font-weight-bold"><span>Remitter ECS Date</span></td>
                                ...
                                <tr class="pay_mode_cheque" style="display:none;">
                                    <td class="text-left font-weight-bold"><span>Remitter Cheque No</span></td>
                                    <td><input type="text" id="cheque_no" name="cheque_no"
                                            class="form-control form-control-sm w-50"
                                            value="<?php echo htmlspecialchars(isset($post_data_array['cheque_no']) && $post_data_array['cheque_no'] != '' ? $post_data_array['cheque_no'] : "");?>" maxlength="6"
                                            /></td>
                                </tr>
                                <!-- Cheque Date -->
                                <tr class="pay_mode_cheque" style="display:none;">
                                    <td class="text-left font-weight-bold"><span>Remitter Cheque Date</span></td>
                                ...
                                <tr>
                                    <td class="text-left font-weight-bold"><span>Remitter Name and Address</span></td>
                                    <td>
                                ...
                                <tr id="cash_from_type_container">
                                    <td class="text-left font-weight-bold"><span id="pay_mode_label">Cash From</span></td>
                                    <td>
                                        <div class="custom-control custom-radio custom-control-inline">
                                            <input type="radio" id="cash_from_type_accounts" name="cash_from_type" value="Accounts"
                                        ...
                                        &nbsp;&nbsp;&nbsp;
                                        <div class="custom-control custom-radio custom-control-inline">
                                            <input type="radio" id="cash_from_type_collection" name="cash_from_type" value="Collection"
                                                class="custom-control-input" <?php
                                ...
                                <tr id="cash_coll_date_row">
                                    <td class="text-left font-weight-bold"><span id="pay_mode_label_date">Cash Collection Date</span></td>
                                ...
                                <!-- Amount -->
                                <tr id="cash_coll_bill_collector">
                                    <td class="text-left font-weight-bold"><span>Bill Collector Name</span></td>
                                    <td>
                                        <select id="bill_collector_name" name="bill_collector_name" class="form-control form-control-sm w-50">
                                            <option value="">Choose</option>
                                            <?php
                                            $sel_payment_type = "SELECT 
                                                b.user_profile_id,
                                                CONCAT_WS(' ', b.user_first_name, b.user_last_name) AS full_name,a.user_profile_id
                                            FROM
                                                (SELECT user_profile_id, dcode, tpcode 
                                                FROM security.t_users_user_data 
                                                WHERE dcode = :dcode 
                                                AND tpcode = :lbcode 
                                                AND del_flag IS NULL and role_code=:role_code) a
                                            JOIN
                                                (SELECT user_profile_id, user_first_name, user_last_name 
                                                FROM security.t_user_profile 
                                                WHERE del_flag IS NULL) b
                                            ON a.user_profile_id = b.user_profile_id;";
                                            $sel_payment_type_res = $this->prepare($sel_payment_type, array(":dcode"=>$dcode, ":lbcode"=>$lbcode,":role_code"=>29), 2);
                                            foreach ($sel_payment_type_res as $sel_payment_type_row) {
                                                ?>
                                                 <option value="<?php echo $sel_payment_type_row['user_profile_id']; ?>">
                                                    <?php echo $sel_payment_type_row['full_name']; ?>
                                                </option>
                                                <?php
                                            }
                                            ?>
                                ...
                                <tr id="cash_coll_amt_row">
                                    <td class="text-left font-weight-bold"><span>Chalan Amount</span></td>
                                    <td>
                                  ...
                                  <div class="col-md-6">
                                    <table class="table table-bordered m-0 p-0 table-striped tndtp_form_table">
                                        ...
                                        <tr>
                                            ...
                                            <td scope="col">
                                            <?php $debit_code = $this->Select_Account_Head_Code(2,7);  ?>
											
                                             <?php $credit_code = $this->Select_Account_Head_Code(1,7); 
                                             //print_r($credit_code);die;
                                             ?>
                                                <select id="debit_bank_code" name="debit_bank_code"
													...
													<?php 
                                                    //print_r($debit_code);die;
                                                    foreach ($debit_code as $row): ?>
                                        ...
                                        <tr>
                                            ...
                                            <td scope="col">
                                                ...
                                                <input type="hidden" id="debit_total_amount" name="debit_total_amount"
                                                    class="form-control form-control-sm number_field" />
                                                    <input type="hidden" id="debit_edit_old_amount" value="0">
                                            </td>
                                        </tr>
                                        <tr>
                                            <td colspan="2">
                                        ...
                                        <tr>
                                            ...
                                            <td scope="col">
                                                ...
                                                <input type="hidden" id="credit_total_amount" name="credit_total_amount"
                                                    class="form-control form-control-sm number_field" />
                                                    <input type="hidden" id="credit_edit_old_amount" value="0">
                                            </td>
                                        </tr>
                                        <tr>
                                            <td colspan="2">
        ...
        $this->Template($this->getCurrentUserTemplate() != "" ? $this->getCurrentUserTemplate() : "Template1", "Adjust Triplicate Chalan Form", $ob_output_main_contents, array(), array('page_id' => 12));
    }
    ...
    {   //print_r($save_data);die();
        ...
        //code for adding debit account breakup when paymode is cash:
         ...
         if (isset($save_data['pay_mode']) && $save_data['pay_mode'] == "1") {
            ...
            $res1 = $this->prepare($save_query, [
                ":acc_type" => 1,
                ":acc_code" => 865,//static account_head_id , taken from local db , change this value if needed
                ":debit_acc_head" => 'General Fund - Cash Account',//static account_head_name , change this value if needed
        ...
        $rc_chalan_no=$rc_serial_no. '/' . $fin_year;
        //final_save_query
        $save_query = "select * from " . $pp_assessment_initiation . "( :dcode, :lbcode, :rc_serial_no,:rc_chalan_no,:rc_date, :cash_from_type, :cash_coll_date, :tax_type, :bank_code,:bank_branch_id, :cheque_no, :cheque_date, :ecs_no, :ecs_date, :dd_no, :dd_date, :pay_mode, :remitter_name_address, :narration, :isactive, :user_name, :ip_address, :edit_id, :del_id, :fin_year,:total_amount,:credit_tot_amount,:debit_tot_amount)";
        $res1 = $this->prepare($save_query, array(":dcode" => $dcode, ":lbcode" => $lbcode, ":rc_serial_no" => $rc_serial_no,":rc_chalan_no" => $rc_chalan_no, ":rc_date" => $rc_date, ":pay_mode" => $pay_mode, ":tax_type" => $tax_type, ":cheque_no" => $cheque_no, ":cheque_date" => $cheque_date, ":bank_code" => $bank_code, ":bank_branch_id" => $bank_branch_id, ":dd_no" => $dd_no, ":dd_date" => $dd_date, ":ecs_no" => $ecs_no, ":fin_year" => $fin_year, ":ecs_date" => $ecs_date, ":remitter_name_address" => $remitter_name_address, ":cash_from_type" => $cash_from_type, ":cash_coll_date" => $cash_coll_date, ":narration" => $narration, ":isactive" => 1, ":user_name" => $user_name, ":ip_address" => $ip_address, ":edit_id" => 0, ":del_id" => 0,":total_amount"=>$credit_amount,":credit_tot_amount"=>$credit_amount,":debit_tot_amount"=>$debit_amount), 4);

        $open_balance_update=new Account_head_balance();
        if (!isset($res1->errorInfo)) {
            $inserted_id = $res1['sp_adjust_triplicate_chalan'];
            $fin_year=$this->getFinYear();
            $latest_date_query = "update accounts_master.t_triplicate_accounthead_breakup set challan_id=:inserted_id   WHERE del_flag IS NULL and tc_serial_no=:triplicate_chalan_no and dcode=:dcode and lbcode=:lbcode and fin_year=:fin_year";
            $sel_dname_res = $this->prepare($latest_date_query, array(":triplicate_chalan_no" => $rc_serial_no, ":dcode" => $dcode, ":lbcode" => $lbcode, ":inserted_id" => $inserted_id,":fin_year"=>$fin_year), 4);

            
            $this->commit();
            try{

                $open_balance_update->update_triplicate_chalan_head_amount($rc_serial_no,false);
            }
    ...
    $url = $site_data->website_url . '/project/forms/masters/triplicate.php?id=' 
        . base64_encode($res1['sp_adjust_triplicate_chalan']);
...
if (!isset($_POST['cmd'])) {
    ...
    if ($cmd == 4) {
        ...
        if (!isset($_POST["edit_id"])) {
            $latest_date_query = "SELECT count(*) as cnt FROM accounts_master.t_triplicate_accounthead_breakup WHERE del_flag IS NULL and tc_serial_no=:triplicate_chalan_no and acc_code=:account_code and dcode=:dcode and lbcode=:lbcode";
        ...
        ?>
            ...
            if (!empty($res1)) {
                ...
                $triplicate_chalan_no = $decoded['accounthead_breakup_id']['tc_serial_no'];



                $latest_date_query = "SELECT accounthead_breakup_id,acc_amount,acc_code FROM accounts_master.t_triplicate_accounthead_breakup WHERE del_flag IS NULL and tc_serial_no=:triplicate_chalan_no and dcode=:dcode and lbcode=:lbcode";
    ...
    if ($cmd == 5) {
        ...
        ?>
            ...
            $sel_dname_res = $propertyassessment->prepare($latest_date_query, array(":accounthead_breakup_id" => $accounthead_breakup_id, ":dcode" => $dcode, ":lbcode" => $lbcode), 4);
    ...
    if ($cmd == 7) {
        ...
        ?>
            <?php
            $save_query = "delete from  accounts_master.t_triplicate_accounthead_breakup where tc_serial_no=:triplicate_chalan_no and dcode=:dcode and lbcode=:lbcode and del_flag is null ";
...
if ($cmd == 12) {
        ...
        $cash_from_type=base64_decode($_POST['cash_from_type']);
            ...
            if(!$propertyassessment->prepareStatus($res1))
            {
                print_r($res1);
            }
        ...
        foreach($sel_qry_res as $sel_qry_row){
            ...
            <tr>
                ...
                <td><?php echo htmlentities($sel_qry_row['debit_amount']); ?>
                    <input type="hidden" name="debit_bank_id" value="<?php echo htmlentities($sel_qry_row['accounthead_breakup_id']);?>" class="bank_id" />
                </td>
                <td>
        ...
        foreach($sel_qry_res as $sel_qry_row){
            ...
            <tr>
                ...
                <td><?php echo htmlentities($sel_qry_row['credit_amount']); ?>
                    <input type="hidden" name="credit_bank_id" value="<?php echo htmlentities($sel_qry_row['accounthead_breakup_id']);?>" class="bank_id" />
                </td>
                <td>
...
if($cmd == 15)
{
    ...
    */
    ...
    $query="SELECT accounthead_breakup_id,debit_amount from accounts_master.t_triplicate_accounthead_breakup where tc_serial_no=:chalan_no and fin_year=:fin_year and lbcode=:lbcode and dcode=:dcode and del_flag is NULL and isactive=1";
