...
class AdjustBankReceiptVoucher  extends ConfigClass
{
    ...
    public function main_content($post_data_array = array())
    {
        ...
        <script type="text/javascript">
            $(document).ready(function() {	
				...
				$(document).on('click', '#btn_debit_edit', function() {
                    ...
                    $.ajax({
						url: "Purchase_journal_vouchers.php",
						type: "post",
						data: {
                            "account_type":btoa(2),
                            "id":btoa(id),
        ...
        <div class="container mt-3">
            ...
            <form action="Purchase_journal_vouchers.php" method="post" enctype="multipart/form-data" autocomplete="off">
                    ...
                    <div class="card-body pl-5 pr-5">
                        ...
                        <table class="table table-bordered m-0 p-0 table-striped tndtp_form_table">
                            <thead class="bg-th-form-dsg">
                                <tr>
                                    <th align="center" scope="col" colspan="12">Purchase Journal Voucher</th>
                                </tr>
                            </thead>
                            <tbody>
                                <tr>
                                    ...
                                    <td  scope="col">
                                    <?php 
                                        $sel_qry="select max(pjv_serial_no) as id from accounts_master.t_pj_voucher where dcode=:dcode and lbcode=:lbcode and del_flag is null  and fin_year=:fin_year;";
                                        $sel_qry_res=$this->prepare($sel_qry, array(":dcode"=>$dcode, ":lbcode"=>$lbcode, ":fin_year"=>$fin_year),4);
                                        $chalan_no =  $sel_qry_res['id']+1 .'/'.$fin_year;
                                        // $del_qry="delete from accounts_master.t_pj_voucher_breakup 
                                        //             where dcode=:dcode 
                                        //                 and lbcode=:lbcode 
                                        //                 and fin_year=:fin_year 
                                        //                 and pjv_serial_no=:pjv_serial_no 
                                        //                 and pjv_id is null;";
                                        $del_qry="update accounts_master.t_pj_voucher_breakup 
                                                    set del_flag='Y',
                                        ...
                                        $del_qry_res=$this->prepare($del_qry, array(
                                            ...
                                            ":lbcode"=>$lbcode, 
                                            ":fin_year"=>$fin_year, 
                                            ":pjv_serial_no"=>$sel_qry_res['id']+1,
                                            ":username"=>$this->getCurrentUser(),
                                            ":ipaddress"=>$this->getIpAddress()
                                        ),4);
                                        echo $chalan_no;                                     
                                        ?>
                                        <input type="hidden" id="pjv_chalan_no" name="pjv_chalan_no" class="form-control w-50 form-control-sm" value="<?php echo $chalan_no; ?>"/>
                                        <input type="hidden" id="pjv_serial_no" name="pjv_serial_no" class="form-control w-50 form-control-sm" value="<?php echo $sel_qry_res['id']+1; ?>"/>
                                    </td>
                                </tr>
                                <tr>
...
<select id="debit_bank_code" name="debit_bank_code" class="form-control form-control-sm mb-2">
    ...
    <?php  $res = $this->Select_Account_Head_Code(1,4);
//     $query = "
//     SELECT 
...
//     */
//      $res = $this->prepare($query, [
//         ":isactive"=>1,":voucher_id"=>20,":account_type_id"=>2
//     ],2);
    foreach ($res as $row): ?>
...
<select id="credit_bank_code" name="credit_bank_code" class="form-control form-control-sm mb-2">
    <option value="">Choose</option>
    <?php $res = $this->Select_Account_Head_Code(2,4);
    // $query = "
    //                 SELECT 
    //                     li.account_headid as account_head_id,
    //                     acc_head.account_code,
    ...
    //             ";
    //                     $res = $this->prepare($query, [
    //                             ":isactive"=>1,":voucher_id"=>20,":account_type_id"=>1
    //                         ], 2);
    foreach ($res as $row): ?>
        <option value="<?= $row['account_head_id']; ?>"
                data-code="<?= htmlentities($row['old_code']); ?>"
...
<input type="hidden" id="credit_bank_head" name="credit_bank_head" class="form-control form-control-sm number_field" />
    ...
    public function data_save($save_data)
    {
        ...
        if (isset($save_data['pjv_chalan_no']) && $save_data['pjv_chalan_no']!='') {
            $pjv_chalan_no = $save_data['pjv_chalan_no'];
            $pjv_chalan_no_Validation = $this->Field_Validation(
                array(
                    'Field_Type' => 'number_slash_hyphen',
                    'Field_Value' => $save_data['pjv_chalan_no'],
                    'Field_Name' => 'BRV Chalan Number',
                    'Field_Label_Name' => 'BRV Chalan Number',
                )
            );

            if ($pjv_chalan_no_Validation['Status'] == "Error") {
                $this->main_content(array_merge(array(
                    "STATUS" => "ERROR",
                    "STATUS_TYPE" => "FIELD",
                    "FIELD_NAME" => "pjv_chalan_no",
                    "MESSAGE" => $pjv_chalan_no_Validation['Message']
                ), $save_data));
                exit;
            }
        }else{
        ...
        $res1 = $this->prepare($save_query, array(":dcode" => $dcode, ":lbcode" => $lbcode, ":pjv_no" => $pjv_serial_no, ":pjv_date" => $date, ":fin_year" => $fin_year, ":debit_amount" => $debit_amount, ":credit_amount" => $credit_amount, ":total_amount" => $credit_amount, ":narration" => $narration, ":user_name" => $user_name, ":ip_address" => $ip_address, ":edit_id" => 0, ":del_id" => 0), 4);
        if (!isset($res1->errorInfo)) {
            $inserted_id = $res1['sp_pj_voucher'];
            $latest_date_query = "update accounts_master.t_pj_voucher_breakup set pjv_id=:inserted_id WHERE del_flag IS NULL and pjv_serial_no=:pjv_serial_no and dcode=:dcode and lbcode=:lbcode and fin_year=:fin_year";
            $sel_dname_res=$this->prepare($latest_date_query,array(":pjv_serial_no"=> $pjv_serial_no,":dcode"=> $dcode,":lbcode"=> $lbcode,":inserted_id"=> $inserted_id, ":fin_year"=>$fin_year),4); 
            $account_head_balance_obj=new Account_head_balance();
            try{
                $account_head_balance_obj->update_voucher_head_amount(4,$pjv_serial_no,False);
                $this->commit();
            }
            catch(Exception $e)
            {
            ...
            $site_data = $this->siteData();
            $url = $site_data->website_url . '/project/forms/masters/Purchase_Journal_Voucher_Reciept.php?pjvno=' . base64_encode($inserted_id);
            header("Location: $url");
            exit;   
        } else {
            $this->rollBack();
...
":amount" => $amount, ":fin_year"=>$fin_year, ":pjv_serial_no" =>$pjv_serial_no,":dcode" => $dcode,":lbcode" => $lbcode,":statecode" => 33,":getCurrentUser" => $user_name,":getIpAddress" => $ip_address,":edit_id" => $debit_edit_id,":delete_id" => $debit_delete_id], 4);
            ...
            foreach($sel_qry_res as $sel_qry_row){
                ...
                <tr>
                    ...
                    <td><?php echo htmlentities($sel_qry_row['debit_amount']); ?>
                        <input type="hidden" name="debit_bank_id" value="<?php echo htmlentities($sel_qry_row['pjv_breakupid']);?>" class="bank_id" />
                    </td>
                    <td>
    ...
    if ($cmd == 6) {
        ...
        if(!isset($res1->errorInfo)){
            $sel_qry = "select pjv_breakupid,credit_account_id, credit_account_head, credit_amount, b.account_head_name_en, account_code from (select pjv_breakupid, credit_account_id, credit_account_head, credit_amount from accounts_master.t_pj_voucher_breakup where dcode=:dcode and lbcode=:lbcode and isactive=:isactive and del_flag is null and pjv_serial_no=:pjv_serial_no and account_type=:account_type and fin_year=:fin_year)a left join (SELECT account_head_id, old_account_head_code as account_code, account_head_name_en FROM accounts_master.m_account_head)b on a.credit_account_id=b.account_head_id;";		
            $sel_qry_res=$AdjustBankReceiptVoucher->prepare($sel_qry,array( ":dcode"=>$dcode,  ":lbcode"=>$lbcode, ":isactive"=>1, ":pjv_serial_no"=>$pjv_serial_no, ":account_type"=>1, ":fin_year"=>$fin_year),2);
            ob_start();
            foreach($sel_qry_res as $sel_qry_row){
                ?>                
                <tr>
                    ...
                    <td><?php echo htmlentities($sel_qry_row['credit_amount']); ?>
                        <input type="hidden" name="credit_bank_id" value="<?php echo htmlentities($sel_qry_row['pjv_breakupid']);?>" class="bank_id" />
                    </td>
                    <td>
