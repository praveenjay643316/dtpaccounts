...
$(document).ready(function() {
    ...
    $(document).on('change', '#cmb_sub1', function() {

        if ($('#cmb_sub1').val() != '') {
            var cmb_lvl = $('#cmb_lvl').val();
            var cmb_sub1 = $('#cmb_sub1').val();
            //alert(allocation_type);

            $.ajax({
                url: "class_menu_addnew_lvl2.php",
                type: "post",
                data: {
                    "cmb_lvl": btoa(cmb_lvl),
                    "cmb_sub1": btoa(cmb_sub1),
                    "cmd": btoa(2)
                },
                //data: {"allocation_type":btoa(allocation_type),"cmd":'%00'},
                success: function(data) {
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td><select id="cmb_lvl" name="cmb_lvl"
                                    ...
                                    <?php
$sel_wrkgrp_name="SELECT role_code,role_name FROM security.m_role where del_flag is null";
...
?>
                        ...
                        <tr>
                            <td>
                                <span DisplayLabelID="121">New Sub Menu II</span>
                            </td>
                            <td><select id="cmb_sub2" name="cmb_sub2" class="form-control form-control-sm w-50">
                                    <option value="" DisplayLabelID="255">Choose Scheme</option>
                                    <?php
if(isset($_POST['cmb_lvl']) && $_POST['cmb_lvl']!='')
{
...
?>
                                ...
                                <script type="text/javascript">
                                document.getElementById('cmb_sub2').value =
                                    '<?php echo htmlentities(isset($_POST['cmb_sub2'])?$_POST['cmb_sub2']:''); ?>';
                                </script>
                            </td>
                        </tr>
...
class=" btn btn-sm btn-primary">Report View</a> -->
 ...
 $sqlmax="SELECT menu_id,smenu_id,user_id,dept_id,smenu_desc, smenu_url,menu_order_no,report_no,report_form_no,rflag,responsive_support, ROW_NUMBER() OVER(PARTITION BY user_id 
            ...
            <div class="col-lg-12 col-ml-12">
                <table class=" table-bordered tndtp_form_report_table" id="Result_table">
                    ...
                    <tbody>
                        ...
                        $cmlvl = $_REQUEST['cmb_lvl'];
                        $cmb_sub1 = $_REQUEST['cmb_sub1'];
                               $list_com = "SELECT menu_id,smenu_id,user_id,dept_id,ssmenu_desc, ssmenu_url,menu_order_no,report_no,report_form_no,rflag,responsive_support, ROW_NUMBER() OVER(PARTITION BY user_id 
ORDER BY user_id, smenu_id) AS rep_no from security.m_submenu2 where user_id=:user_id and ssmenu_id=:cmb_sub1 and  del_flag is null;";
                        ...
                        <tr>
                            ...
                            <td align="left">
    <?php echo htmlentities($row['ssmenu_desc']) . htmlentities($row['rep_no']); ?>
</td>
    ...
    <?php echo ($row['report_no'] == 1) ? 'R' : 'F'; ?>
                            ...
                            <td align="left"><a
                                    class="btn btn-warning btn-sm"><i class="fa fa-pencil pe-1 me-1"
...
<?php
	...
	if(isset($save_data['cmb_lvl']) && $save_data['cmb_lvl']!='')
        ...
        if(isset($save_data['rad_on_off']) && $save_data['rad_on_off']!='')
        {
            ...
            $cmb_programmer=$save_data['cmb_programmer'];
            $purpose=$save_data['purpose'];




 $sqlmx="SELECT max(smenu_id) as smxid FROM security.m_submenu2 WHERE del_flag IS NULL and user_id=:cmb_lvl;";
	...
	$user_name = $this->getCurrentUser();
	$save_query1="select * from security.sp_submenu2(:menu_id,:smenu_id,:user_id,:dept_id,:txt_mnname,:txt_mnurl,:menu_order_no,:report_form_flag,:report_form_no,:rad_on_off,:cmb_app_web,:cmb_sub1,:table_name,:cmb_programmer,:purpose,:user_name,:ip_address,:edit_id)";
               $res1=$this->prepare($save_query1,array(":menu_id"=>01,":smenu_id"=>$maxid,":user_id"=>$cmb_lvl,":dept_id"=>01,":txt_mnname"=>$txt_mnname,":txt_mnurl"=>$txt_mnurl,":menu_order_no"=>$menu_order_no,":report_form_flag"=>$report_form_flag,":report_form_no"=>$report_form_no,":rad_on_off"=>$rad_on_off,":cmb_app_web"=>$cmb_app_web,":cmb_sub1"=>$cmb_sub1,":table_name"=>$table_name,":cmb_programmer"=>$cmb_programmer,":purpose"=>$purpose,":user_name"=>$user_name,":ip_address"=>$ip_address,":edit_id"=>0),4); 
...
else
{
	try
    {
		if(isset($_POST['cmd']) && $_POST['cmd']!=''){
            ...
            if($cmd==2){          
                $cmb_lvl=base64_decode($_POST['cmb_lvl']);
                $cmb_sub1=base64_decode($_POST['cmb_sub1']);
                $sel_role_qry="select ssmenu_id,smenu_id,user_id,ssmenu_desc from security.m_submenu2 where user_id=:user_id and smenu_id=:smenu_id and del_flag is null"; 
                $sel_role_qry_res=$propertyassessment->prepare($sel_role_qry,array(":user_id"=>$cmb_lvl,":smenu_id"=>$cmb_sub1),2);
                ...
                {
                ?>
                    <option value="<?php echo htmlentities($sel_street_details_row['ssmenu_id']); ?>">
                    <?php echo htmlentities($sel_street_details_row['ssmenu_desc']); ?></option>
                    <?php
                }
                exit;
            }
