...
class ForceChangePassword extends ConfigClass
{
	...
	public function changePassword()
	{
		...
		if ($res["no_of_days_from_update"] >= $this->siteData()->FORCE_PASSWORD_CHANGE) 
		{
			$_SESSION['change_password_required']='Y';	
			
			$this->Template("Plaintemplate", "User Role", str_repeat("<br>", 5) . $this->change_password(array(),array()), array(
                        array(
                            "name" => "User Role",
                        ),
                    ));
    ...
    {             
            $this->Template("Plaintemplate", "User Role", str_repeat("<br>", 5) . $this->change_password_view($data_array_post, $data_array_get), array(
                array(
                    "name" => "User Role",
                ),
            ));
